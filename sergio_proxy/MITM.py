...
class MITM:
    ...
    def __init__(self,req_funcs,reply_funcs):
        self.req_funcs = req_funcs + [self.restrict_encodings]
        self.reply_funcs = reply_funcs

    def __str__(self):
        return '\n'.join(("Request functions: " + ', '.join(f.__name__ for f in self.req_funcs),
            "Reply functions: " + ', '.join(f.__name__ for f in self.reply_funcs)))
    ...
    def restrict_encodings(self):
        enc = set(['gzip'])
        headers = self.req.getAllHeaders()
        if 'Accept-Encoding' in headers:
          encodings = ','.join(filter(lambda x: x in enc, headers['Accept-Encoding'].split(',')))
          self.req.setHeader('Accept-Encoding', encodings)
